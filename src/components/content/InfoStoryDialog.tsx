...
interface StoryPage {
  key: "about" | "trailer" | "cast" | "facts" | "boxoffice" | "ratings" | "related" | "collection";
  label: string;
}
...
const tmdbImageUrl = (path?: string, size: "w342" | "w780" | "original" = "w780") =>
  path?.startsWith("http") ? path : path ? `https://image.tmdb.org/t/p/${size}${path}` : undefined;
...
export function InfoStoryDialog({
  ...
  const hasBoxOffice = Boolean(data?.productionBudget || data?.worldwideGross || data?.domesticGross || data?.openingWeekendGross);
  ...
  const pages = useMemo<StoryPage[]>(() => {
    ...
    if (data?.trailerKey || data?.trailerUrl || data?.trailers?.length) result.push({ key: "trailer", label: "Trailer" });
  ...
  const backdrops = useMemo(() => {
    const paths = (data?.backdropPaths ?? []).map((path) => tmdbImageUrl(path, "original"));
    const trailerThumb = data?.trailerThumbnail;
    return Array.from(new Set([...paths, trailerThumb, backdrop].filter(Boolean))) as string[];
  ...
  const facts = data ? [
    ...
    data.watchlistCount ? { icon: LuUsers, label: "Watchlist", value: data.watchlistCount.replace("Added by ", "") } : null,
    data.totalSeasons ? { icon: LuTv, label: "Seasons", value: `${data.totalSeasons} ${data.totalSeasons === 1 ? "season" : "seasons"}` } : null,
    data.totalEpisodes ? { icon: LuTv, label: "Episodes", value: `${data.totalEpisodes} ${data.totalEpisodes === 1 ? "episode" : "episodes"}` } : null,
    data.upcomingSeason ? { icon: LuCalendar, label: "Upcoming", value: data.upcomingSeason } : null,
  ...
  const handleHalfClick = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    if (target.closest("[role='button'], button, a, iframe, video, input, select, textarea, .info-story-navigation, .info-story-video")) return;
  ...
  return (
    <div className="info-story-dialog" role="dialog" aria-modal="true" aria-label={`${title} information`} onClick={handleHalfClick}>
      ...
      <FocusableButton className="info-story-close" onClick={onClose} focusKey="STORY_CLOSE" title="Close"><LuX size={24} /></FocusableButton>
      ...
      {!data ? (
        <div className="info-story-state">
          {isFetching ? (
            ...
            <>
              ...
              <p>{error instanceof Error ? error.message : "Metadata could not be loaded."}</p>
        ...
        <AboutPage data={data} title={title} description={description} backdrops={backdrops} backdropIndex={backdropIndex} />
...
function AboutPage({ data, title, description, backdrops, backdropIndex }: { data: any; title: string; description?: string; backdrops: string[]; backdropIndex: number }) {
  return (
...
function TrailerPage({ data }: { data: any }) {
  ...
  const trailers = useMemo(() => {
    ...
    if (data.trailerUrl) return [{ id: "1", name: data.trailerName, url: data.trailerUrl, thumbnail: data.trailerThumbnail }];
    if (data.trailerKey) return [{ id: "1", name: data.trailerName, youtubeKey: data.trailerKey }];
    return [];
  }, [data.trailers, data.trailerUrl, data.trailerName, data.trailerThumbnail, data.trailerKey]);

  const activeVideo = trailers[Math.min(trailerIndex, trailers.length - 1)] || trailers[0];
    ...
    <div className="info-story-single-page info-story-trailer-page">
      <header className="info-story-page-heading">
        ...
        <div>
          ...
          <h1>{trailers.length > 1 ? `Trailer (${trailerIndex + 1}/${trailers.length})` : "Trailer"}</h1>
        </div>
      </header>
      ...
      <div className="info-story-trailer-footer">
        <strong>{activeVideo?.name || data.trailerName || `${data.title} trailer`}</strong>
        {activeVideo?.youtubeKey && (
          <FocusableButton className="info-story-youtube" onClick={() => void openUrl(`https://www.youtube.com/watch?v=${encodeURIComponent(activeVideo.youtubeKey || "")}`)}>
            Open in YouTube
          </FocusableButton>
        )}
      </div>
          ...
          >
            <div className="info-story-cast-poster-wrap">
              {tmdbImageUrl(person.profilePath, "w342") ? (
                <img src={tmdbImageUrl(person.profilePath, "w342")} alt={person.name} />
              ) : (
                <div className="info-story-person-placeholder"><LuUsers size={28} /></div>
              )}
            </div>
...
function BoxOfficePage({ data }: { data: any }) {
  const stats = [
    ...
    { label: "Opening Weekend", value: data.openingWeekendGross, icon: LuCalendar },
  ...
  return (
    <div className="info-story-single-page">
      <header className="info-story-page-heading"><LuDollarSign size={30} /><div><span>{data.title}</span><h1>Box Office</h1></div></header>
      <div className="info-story-boxoffice">
        {stats.map(({ label, value, icon: Icon }) => {
          const pct = maxValue > 0 ? Math.round((value / maxValue) * 100) : 0;
          return (
            <div className="info-story-boxoffice-row" key={label}>
              <div className="info-story-boxoffice-label"><Icon size={18} /><span>{label}</span></div>
              <div className="info-story-boxoffice-bar-track">
                <div className="info-story-boxoffice-bar-fill" style={{ width: `${pct}%` }} />
              </div>
              <strong className="info-story-boxoffice-value">{formatCurrency(value)}</strong>
            </div>
          );
        })}
      </div>
...
function RatingsPage({ data }: { data: any }) {
  ...
  const hasReview = Boolean(data.featuredReview?.text || data.featuredReview?.summary);

  return (
    <div className={`info-story-single-page ${hasReview ? 'info-story-ratings-page' : ''}`}>
      <header className="info-story-page-heading">
        <LuChartBar size={30} />
        <div><span>{data.title}</span><h1>User reviews</h1></div>
      </header>
            ...
            {histogram.map((entry: any) => {
              ...
              return (
                <div className="info-story-histogram-col" key={entry.rating}>
                  ...
                  <span className="info-story-histogram-label">{entry.rating}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="info-story-featured-review">
            ...
            {data.featuredReview.rating && (
              <div className="info-story-featured-review-rating">
                <LuStar size={16} fill="currentColor" /> {data.featuredReview.rating}/10
              </div>
            )}
            {data.featuredReview.summary && <strong>{data.featuredReview.summary}</strong>}
            <p
              ref={reviewTextRef}
              className={isReviewExpanded ? "info-story-featured-review-expanded" : ""}
            >
              {data.featuredReview.text}
            </p>
    ...
    <div className="info-story-single-page info-story-facts-page">
      ...
      <div className="info-story-fact-columns">
        {data.creators?.length > 0 && (
          <section><h2>{data.mediaType === "tv" ? "Created by" : "Directed by"}</h2><p>{data.creators.join(" · ")}</p></section>
        )}
        {(data.networks?.length > 0 || data.companies?.length > 0) && (
          <section><h2>Production</h2><p>{[...(data.networks ?? []), ...(data.companies ?? [])].join(" · ")}</p></section>
        )}
        {data.awardsText && <section><h2>Awards</h2><p>{data.awardsText}</p></section>}
      </div>
      <div className="info-story-chips">{[...data.genres, ...data.keywords].slice(0, 12).map((item: string) => <span key={item}>{item}</span>)}</div>
    </div>
  );
}
...
function RelatedPage({ data }: { data: any }) {
  ...
  return (
    <div className="info-story-single-page info-story-related-page">
      <header className="info-story-page-heading"><LuFilm size={30} /><div><span>{data.title}</span><h1>More like this</h1></div></header>
  ...
  return (
    <div className="info-story-single-page info-story-collection-page">
      <header className="info-story-page-heading">
        ...
        <div><span>{data.title}</span><h1>{data.collectionTitle || (data.mediaType === "tv" ? "Seasons" : "Collection")}</h1></div>
      </header>
      <div className="info-story-collection-grid">
        {items.slice(0, 12).map((item: any) => {
          const img = tmdbImageUrl(item.imagePath, "w342") || (item.imagePath?.startsWith("http") ? item.imagePath : undefined);
          return (
            <article className="info-story-collection-card" key={item.id}>
              <div className="info-story-collection-poster-wrap">
                {img ? (
                  ...
                  <div className="info-story-poster-placeholder"><LuFilm size={28} /></div>
                )}
              </div>
